...
if df.empty:
    st.warning("No hay datos aún. Ejecuta el monitor manualmente desde GitHub Actions.")
    st.stop()
...
if not df_filtrado.empty:
    ...
    st.dataframe(df_filtrado.sort_values('timestamp', ascending=False).head(15), use_container_width=True)
else:
    st.warning(f"No hay datos para **{estacion_sel}** en la fecha seleccionada.")

st.caption("Actualizado cada 5 minutos por GitHub Actions")
