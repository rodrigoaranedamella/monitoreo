...
# --- LÓGICA DE GRABACIÓN FORZADA ---
def apoyo_persistencia_forzada():
    """Graba siempre que la app se refresque (cada 2.5 min)"""
    try:
        ...
        for nombre in ESTACIONES:
            ...
            # Si está online en ZeroTier (visto hace < 10 min por margen de error)
            if (ahora_ms - last_seen) / 1000 < 600:
                datos_grabado.append({
                    "device": nombre,
                    "estado": True,
                    "duracion_min": 2.5, # Ajustado a tu tiempo de refresco
                    "timestamp": timestamp_chile
                })

        if datos_grabado:
            supabase.table("historial_conexiones").insert(datos_grabado).execute()
    except Exception:
        pass 

# Ejecutar grabación cada vez que se carga la página
apoyo_persistencia_forzada()
    ...
    for estacion in ESTACIONES:
        try:
            ...
            if res.data:
                ...
                esta_online = diff_min < 15 # Si grabamos cada 2.5 min, 15 min es margen de sobra
...
def cargar_grafica_timeline(device, fecha):
    try:
        ...
        for i in range(len(df)):
            ...
            timeline.append({'Inicio': curr, 'Fin': curr + timedelta(minutes=2.5), 'Estado': 'Conectado'})
            if i < len(df) - 1:
                prox = df.iloc[i+1]['timestamp']
                if (prox - curr).total_seconds() / 60 > 10:
                    timeline.append({'Inicio': curr + timedelta(minutes=2.5), 'Fin': prox, 'Estado': 'Desconectado'})
...
with col_t:
    if not df_act.empty: st.table(df_act)
with col_c:
